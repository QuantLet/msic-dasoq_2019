...
### STEP 4: LOAD DATA
#################################################### 
setwd("/Users/ingolfpernice/Documents/msic-dasoq_2019/msic-dasoq_2019_06_mon_reg_plot")
...
### Making Plot data ###
cryptos = table_all_MR[,"Fraction"]
cryptoTotal = sum(cryptos)
cryptoslaunched = table_only_launched_MR[, "Fraction"]
cryptolaunchedTotal = sum(cryptos)
imf = c(40, 24, 82, 46)
imfTotal = sum(imf)
...
dt = data.table(regime, cryptos*100/cryptoTotal, cryptoslaunched*100/cryptolaunchedTotal, imf*100/imfTotal)
                                        # Change it to ggplot's format
dt = melt(dt, id.vars="regime")
                                        # sort columns
dt$regime <- reorder(dt$regime, dt$value)
                                        # Take the data, fill by type (stablecoin/imf)
q = ggplot(dt, aes(x=regime, y=value, fill=variable)) +
    ...
    xlab("Monetary Regime") + ylab("Fraction of projects and countries in \\%") +
    ...
    # scale_x_discrete(labels=regimeLabels) +
                                        # position of ledgend
...
### Write out to file (if necessary) ###
   tikz(file = "./monetary_regimes_comparison.tex", width = 3.4, height = 2.5)
q
 dev.off()
