...
## [<img src="https://github.com/QuantLet/Styleguide-and-FAQ/blob/master/pictures/qloqo.png" alt="Visit QuantNet">](http://quantlet.de/) **msic-dasoq_2019_07_arr_reg_plot** [<img src="https://github.com/QuantLet/Styleguide-and-FAQ/blob/master/pictures/QN2.png" width="60" alt="Visit QuantNet 2.0">](http://quantlet.de/)

```yaml

Name of Quantlet: msic-dasoq_2019_07_arr_reg_plot
...
Keywords: stablecoins, survey data, stability, plot, exchange rate regimes, bitcoin, pricing, economics
...
![Picture1](ERArrangements_comparison.png)
...
### STEP 4: LOAD DATA
#################################################### 
setwd("/Users/ingolfpernice/Documents/msic-dasoq_2019/msic-dasoq_2019_07_arr_reg_plot")
...
theme_set(theme_bw(10))
regimeLabels = c("Inflation T.","Monetary T.","Exch. Rate T.", "Other")
regime = factor(rownames(table_all_MR), levels = rownames(table_all_MR),
                labels = regimeLabels)

arrangementLabels = rownames(table_all_ERA)
arrangement = factor(rownames(table_all_ERA),
                     rownames(table_all_ERA),
                     labels = rownames(table_all_ERA))

### Making Plot Data ###
cryptos = table_all_ERA[,"Fraction"]
cryptoTotal = sum(cryptos)
cryptoslaunched = table_only_launched_ERA[, "Fraction"]
cryptolaunchedTotal = sum(cryptoslaunched)
imf = c(13.0, 39.6, 20.8, 16.1, 10.4) # already in percent


### Plotting ###
                                        # Gather data in a data.table/data.frame
dt = data.table(arrangement,
                cryptos*100/cryptoTotal,
                cryptoslaunched*100/cryptoTotal,
                imf)
...
q = ggplot(dt, aes(x=arrangement, y=value, fill=variable)) +
    ...
    xlab("Exchange Rate Arrangement") + ylab("Fraction of projects and countries in \\%") +
  
                                        # Set the legend accordingly
    scale_fill_manual(name="Type", values=palette, labels = c("All Proj.", "Impl. Proj.", "IMF")) +
  
    
                                        # As well as x axis labels
    # scale_x_discrete(labels=arrangementLabels) + # you again broke my stuff!
                                         # position of ledgend
...
tikz(file = "./ERArrangements_comparison.tex", width = 3.4, height = 3)
q
dev.off()
